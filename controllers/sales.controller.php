...
class ControllerSales {
	...
	public static function ctrCreateSale() {
		if (isset($_POST["newSale"])) {
			...
			// 		$printer->text("ZMW ".number_format($value["price"],2)." Und x ".$value["quantity"]." = ZMW ".number_format($value["totalPrice"],2)."\n");

			// 	 }

			// 	$printer -> feed(1); //We feed paper 1 time*/			
				
			// 	 $printer->text("NET: ZMW ".number_format($_POST["newNetPrice"],2)."\n"); //net price

			// 	$printer->text("TAX: ZMW ".number_format($_POST["newTaxPrice"],2)."\n"); //tax value

			//  	$printer->text("--------\n");

			// 	 $printer->text("TOTAL: ZMW ".number_format($_POST["saleTotal"],2)."\n"); //ahora va el total
