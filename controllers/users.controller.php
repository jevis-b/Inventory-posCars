	...
	=============================================*/
		...
		if (isset($_POST["loginUser"])) {
			if (preg_match('/^[a-zA-Z0-9]+$/', $_POST["loginUser"]) && 
				preg_match('/^(?=.*[A-Z])(?=.*\d)(?=.*[^\w\s]).{8,}$/', $_POST["loginPass"])) {
				...
				if(is_array($answer) && $answer["user"] == $_POST["loginUser"] && $answer["password"] == $encryptpass){
					if($answer["status"] == 1){
				...
				echo '<br><div class="alert alert-danger">Invalid password format. Password must include at least 6 characters, a number, an uppercase letter, and a special character.</div>';
			}
		}
	}
...
static public function ctrCreateUser(){
		if (isset($_POST["newUser"])) {
			if (preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["newName"]) &&
				...
				preg_match('/^(?=.*[A-Z])(?=.*\d)(?=.*[^\w\s]).{6,}$/', $_POST["newPasswd"])){
				...
				echo '<script>
					...
					swal({
						type: "error",
						title: "Invalid password format or blank fields",
						showConfirmButton: true,
						confirmButtonText: "Close"
